...
class _EvaluationResponse:
    ...
    def from_dict(cls, payload: dict) -> "_EvaluationResponse":
        ...
        raw_score = payload.get("completion_score", payload.get("match", 0))
...
- A score >= 0.6 is treated as a successful execution.
    ...
    async def evaluate(self, goal: Goal, execution: GoalExecution) -> ExecutionOutcome:
        try:
            ...
            if response.match is not None:
                completion_score = 1.0 if response.match else 0.0
        ...
        observation_id = self.world_model.record_goal_execution(goal, execution, outcome)
        self.world_model.update_goal_status(goal.id, new_status)
        if outcome.new_theory and outcome.new_theory.strip():
            self.world_model.add_theory(outcome.new_theory.strip(), source_observation_id=observation_id)
        return outcome
